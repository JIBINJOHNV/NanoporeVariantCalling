...
SampleWIse_Longshot_Nanopore_Mrged_ColumnReorganisation="python /data/NGC_Data/SampleWIse_Longshot_Nanopore_Mrged_ColumnReorganisation.py"
...
((i=i%N)); ((i++==0)) && wait  
zcat ${Data}${barcod}/*gz > FastQfiles/${barcod}.fq & done  ;wait
...
longqc sampleqc -x ont-rapid -m 2 -p 5 -o /output/${Samplename}/ /input/FastQfiles/${Samplename}.fq & done  ;wait

#A) QC checking of Raw FastQ data using Fatqc
mkdir -p QC/FastQC/Raw_Fastq_QC/
for barcod in ${Barcodes[@]} ; do
((i=i%N)); ((i++==0)) && wait
fastqc -t 35 -o QC/FastQC/Raw_Fastq_QC/ FastQfiles/${barcod}.fq & done  ;wait
...
-o CleanedFastQ/PoreChop/"${barcod}_PoreChop.fq" --discard_middle & done  ;wait
...
longqc sampleqc -x ont-rapid -m 2 -p 5 -o /output/${Samplename}/ /input/CleanedFastQ/TrimmerLarge/${Samplename}_Pore_TrimmerLarge.fastq & done  ;wait
...
for barcod in ${Barcodes[@]} ; do
((i=i%N)); ((i++==0)) && wait
fastqc -t 35 -o QC/FastQC/TrimmerLarge_Fastq_QC/ CleanedFastQ/TrimmerLarge/${Samplename}_Pore_TrimmerLarge.fastq & done ;wait
...
for bam in ${Barcodes[@]} ; do
((i=i%N)); ((i++==0)) && wait
mosdepth -F 772 --fast-mode --by ${T_RegionFolder}${T_Region} \
...
--output QC/AlignmentQC/Mosdepth/TargetRegion/${bam}_MosdepthCoverage.html & done ; wait
...
--ref ${RefFolder}${fasta} \
--out Final_VcfFiles/longshot/Raw/"${bam}_longshot.vcf" & done ; wait
...
((i=i%N)); ((i++==0)) && wait
${longshot_add_DP_AF} \
-VcfFile Final_VcfFiles/longshot/Raw/"${bam}_longshot.vcf" & done ; wait 
...
tabix -p vcf ${Samplename}_longshot_filtered_Normalised.vcf.gz ;done ; wait  
...
-nastring . -polish -xref ${AnnovarDb}gene_fullxref.txt & done ; wait  
...
for SAMPLENAME in ${Barcodes[@]} ; do 
((i=i%N)); ((i++==0)) && wait
	${AnnovarFiltering}  --AnnovarFile ${SAMPLENAME}_longshot_filtered_Normalised.hg38_multianno.txt & done ; wait  
...
for SAMPLENAME in ${Barcodes[@]} ; do 
	...
	sed -i 's/\S*Otherinfo\S*//g' ${SAMPLENAME}_longshot_filtered_Normalised.hg38_multianno_DesiredColumns_Filtered.tsv & done ; wait
...
((i=i%N)); ((i++==0)) && wait
 ...
 ${longshot_multianno} \
 -AnnovarOutputFile ${SAMPLENAME}_longshot_filtered_Normalised.hg38_multianno_DesiredColumns_Filtered.tsv & done ; wait 
...
docker run  -it \
  ...
  --sample_name=${bam} \
   --gvcf  \
  --output=${OUTPUT_DIR}  & done ; wait     #if want to generate  gvcf use --gvcf    
...
for Samplename in ${Barcodes[@]} ; do
((i=i%N)); ((i++==0)) && wait
tabix --force -p vcf ${Samplename}_Clair3_filtered.vcf.gz ; done
...
tabix --force -p vcf ${Samplename}_Clair3_filtered_Normalised.vcf.gz ;done 
...
sed 's/region/f_NAME/' QC/AlignmentQC/Mosdepth/TargetRegion/20X.csv  > 20X.csv
...
${SampleWIse_Longshot_Nanopore_Mrged_ColumnReorganisation}
${Amplicon_Variant_SampleSummary}


gzip FastQfiles/*

mkdir -p Data/Folder1
mkdir -p Data/Folder2
mkdir -p Data/Folder1/Vcf_Files

mv Total*.csv Data
cp -r FastQfiles Data/Folder1/
cp -r Final_VcfFiles/* Data/Folder1/Vcf_Files
cp -r Annovar_Output/ Data/Folder1/
cp -r QC/ Data/Folder1/
cp -r MultiQC/ Data/Folder1/
cp -r SampleWise_Annovar/ Data/Folder2
cp -r BamFiles Data/Folder1/


rm -rf Data/Folder2/SampleWise_Annovar/Present
rm -rf Data/Folder2/SampleWise_Annovar/NotPresent