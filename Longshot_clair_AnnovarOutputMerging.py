...
clairdf = clairdf.loc[:, ~clairdf.columns.str.contains('^Unnamed')]
longdf = longdf.loc[:, ~longdf.columns.str.contains('^Unnamed')]
...
if CSampleName!=LSampleName:
    print(CSampleName + " and " + LSampleName + " not identical")
    sys.exit()



if 'RefinedGenotype' not in clairdf.columns:
    clairdf['RefinedGenotype']="NA"

if 'VariantAlleleFrequency' not in clairdf.columns:
    clairdf['VariantAlleleFrequency']="NA"


if 'RefinedGenotype' not in longdf.columns:
    longdf['RefinedGenotype']="NA"

if 'VariantAlleleFrequency' not in longdf.columns:
    longdf['VariantAlleleFrequency']="NA"
...
try:
    clairdf=clairdf.drop(['QUAL.1','FILTER.1'],axis=1)
except:
    pass
try:
    longdf=longdf.drop(['QUAL.1','FILTER.1'],axis=1)
except:
    pass



CommonColumnsdf = pd.concat([clairdf.drop(claircolumns,axis=1),longdf.drop(longcolumns,axis=1)]).drop_duplicates()
...
print(LSampleName+"_"+outputname+'_clair_long.csv')
